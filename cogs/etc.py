...
""",
    ...
    @commands.command()
    async def steam(self, ctx, nickname, cache_data=None):
        mode = "profiles"
        if cache_data == None:
            mode = "id"
        message = ctx.message
        html = requests.get(
            "https://steamcommunity.com/" + mode + "/" + nickname + "/"
        ).text
        try:
            ...
            try:
                html.split("This profile is private.")[1]
            except:
                status = ""
                is_Online = html.split('<div class="profile_in_game_header">')[1].split(
                    "</div>"
                )[0]
                if is_Online == "Currently Offline":
                    ...
                    status = (
                            status
                            + "("
                            + html.split('<div class="profile_in_game_name">')[1].split(
                        "</div>"
                    )[0]
                            + ")"
                    )
                ...
                try:
                    level = html.split('<span class="friendPlayerLevelNum">')[1].split(
                        "</span>"
                    )[0]
                    embed.add_field(name="레벨:", value=level, inline=True)
                    local_data = ""
                    for i in range(3):
                        cache_html1 = html.split('<div class="game_name">')[
                            i + 1
                            ].split("</div>")[0]
                        game_name = cache_html1.split('">')[1].split("</a>")[0]
                        game_time = (
                            html.split('<div class="game_info_details">')[i + 1]
                                .split("on record")[0]
                                .replace("hrs", "시간")
                                .replace("	", "")
                                .replace(" ", "")
                ...
                embed = discord.Embed(
                    title="Error!", description="이 계정은 비공개 계정입니다.", color=0x00AAAA
                )
                await message.channel.send(embed=embed)
        except:
            try:
                ...
                if answer == "The specified profile could not be found.":
                    answer = (
                            "요청하신 계정을 찾을수 없습니다. 만약에 nickname이 없다면 유저코드를 적어주세요.\n "
                            + prefix
                            + "스팀 [유저코드] [유저코드활성여부(아닐경우 미기재)]\n유저코드를 기재 할경우에는 유저코드를 활성화하셔야합니다."
                    )
                embed = discord.Embed(
                    title="Error!", description=answer, color=0x00AAAA
                )
                await message.channel.send(embed=embed)
            except:
                embed = discord.Embed(
                    title="Error!", description="알수없는 오류가 발생하였습니다.", color=0x00AAAA
                )
                await message.channel.send(embed=embed)
        return
